...
class iotData:
    def __init__(self, platform, location, subscription, resourceName, kubernetesVersion, vnetAddressPrefix, vnetSubnetPrefix, networkSecurityGroupName, vnetName, virtualMachineName, virtualMachineSize, destinationPortRange, isARMTemplate, services, aks_uid):
    ...
    for uid in uids:
        ...
        if len(input_properties) > 0:
            ...
            if "security-group" in resource_type:
                ...
                ingress_rules = (re.sub("[\"]","", ingress_rules_input[0]))[1:-1]
                egress_cidr_blocks = (re.sub("[\"]","", egress_cidr_blocks_input[0]))[1:-1]
                egress_rules = (re.sub("[\"]","", egress_rules_input[0]))[1:-1]
                ...
                if len(ingress_with_cidr_blocks_input) > 0:
                    ingress_with_cidr_blocks = (re.sub("[\"]", "", ingress_with_cidr_blocks_input[0]))
                ...
                if len(service_endpoints_input) > 0:
                    service_endpoints = (re.sub("[\"]","", service_endpoints_input[0]))[1:-1]
        ...
        from_s = jmespath.search("linkDataArray[?to==`%d`].from" % key_other, data)
        for p in from_s:
        ...
        while len(group) > 0:
            ...
            if not dependencies_in_not_supported(group_resource_type, dependencies_not_supported):
